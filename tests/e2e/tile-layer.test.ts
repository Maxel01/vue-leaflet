...
test.describe('TileLayer playground page', () => {
    ...
    test('overlay minZoom is respected (tiles disappear below minZoom)', async ({ page }: { page: Page }) => {
        await page.goto('/tile-layer')

        // ensure both sources are present initially
        await page.waitForSelector('.leaflet-tile', { timeout: 15000 })
        await page.waitForFunction(() => {
            const imgs = Array.from(document.querySelectorAll('.leaflet-tile')) as HTMLImageElement[]
            return imgs.some(i => /s3.amazonaws/.test(i.src)) && imgs.some(i => /openstreetmap/.test(i.src))
        }, { timeout: 15000 })
        ...
        await page.waitForSelector('.leaflet-tile[src*="s3.amazonaws"]', { state: 'detached', timeout: 15000 })
    })

    test('overlay maxZoom is respected (tiles disappear above maxZoom)', async ({ page }: { page: Page }) => {
        await page.goto('/tile-layer')
